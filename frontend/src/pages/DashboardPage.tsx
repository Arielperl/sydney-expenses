...
function SecondaryStat({
  ...
  const content = (
    <>
      ...
      {plainValue !== undefined ? (
        <p className="mt-1 text-2xl font-semibold tracking-tight tabular-nums text-stone-900 dark:text-stone-100">
        ...
        <div className={rows.length > 1 ? 'mt-2 space-y-1' : undefined}>
          {rows.map((row) => (
            <p
              key={row.currency}
              className="mt-1 text-2xl font-semibold tracking-tight tabular-nums text-stone-900 dark:text-stone-100"
            >
              {formatCurrency(row.amount, row.currency, i18n.language)}
              {rows.length > 1 && <span className="ms-1 text-xs font-medium text-stone-400">{row.currency}</span>}
            </p>
          ))}
        </div>
      )}
  ...
  const className =
    'block rounded-xl border border-stone-200 bg-white p-4 shadow-sm dark:border-stone-800 dark:bg-stone-900' +
      ...
      {!customRangeIncomplete && !isLoading && !isError && data && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="rounded-xl border border-stone-200 bg-white p-5 shadow-sm lg:col-span-2 dark:border-stone-800 dark:bg-stone-900">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-base font-semibold text-stone-900 dark:text-stone-100">{t('dashboard.revenueTrend')}</h2>
                <div className="inline-flex rounded-lg border border-stone-200 p-0.5 text-xs font-medium dark:border-stone-700">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="rounded-xl border border-stone-200 bg-white shadow-sm lg:col-span-2 dark:border-stone-800 dark:bg-stone-900">
              ...
              {data.recent_sales.length === 0 ? (
                <div className="px-5 py-10 text-center">
                  <p className="text-sm text-stone-500 dark:text-stone-400">{t('dashboard.recentSalesEmpty')}</p>
                  <Link
                    to="/add-sale"
                ...
                <ul className="divide-y divide-stone-100 dark:divide-stone-800">
                  {data.recent_sales.map((sale) => (
                    <li key={sale.id}>
                      <Link
                        to={`/sales/${sale.id}`}
                        className="flex flex-wrap items-center justify-between gap-3 px-5 py-3 hover:bg-stone-50 dark:hover:bg-stone-800/50"
                      >
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          {sale.payment_method && (
                            <span className="hidden text-xs text-stone-400 sm:inline dark:text-stone-500">
