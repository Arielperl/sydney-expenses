    ...
    <div className="rounded-xl border border-zinc-200/80 bg-white p-6 shadow-card dark:border-zinc-800 dark:bg-zinc-900">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 max-w-xl text-xs leading-relaxed text-zinc-500 dark:text-zinc-400">{t('imports.cardcom.description')}</p>
        </div>
        {isOwner && (
          <button
            type="button"
            ...
            className={buttonClasses(!isLoading && cardcomConnections.length === 0 && !showCreate ? 'primary' : 'secondary')}
          >
      ...
      {!isOwner && <p className="mt-4 text-xs text-zinc-500 dark:text-zinc-400">{t('imports.cardcom.ownerOnlyNote')}</p>}
    </div>
  )
}
