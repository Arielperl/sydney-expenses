...
- **Support requests and staff console** — owners and managers use the button in Data Import to send a request tied to their current business. Staff sign in at `https://support.sydneyexpenses.com/support/login`. The database-backed hierarchy is `user` → `support` → `admin` → `superadmin`: support and admin accounts can handle requests and add/remove Grow or Cardcom for a business; only the superadmin can list accounts, create a confirmed Supabase identity with an initial password, change platform roles, or delete access. The initial superadmin is promoted by migration from `arielperl999@gmail.com`. A sole business owner cannot be deleted before ownership is transferred, and the superadmin cannot demote or delete itself. The old `/admin` screen and API are gone. Neither the separate hostname nor an unlisted route is treated as an authorization control: the server checks the role on every staff request. Deleted auth identities retain a disabled, anonymized app-account tombstone to reject any still-valid access token while business sales remain intact. Passwords are handled only by Supabase Auth and are never stored in the Sydney database.
