...
- **Local Demo Area** (`/demo`) — simulate realistic sale scenarios (successful/pending/failed payment, a document that fails to issue, a partial or full refund, ILS/USD/EUR) entirely from the UI, no terminal required. Every simulated sale is created through the same ingestion path a real webhook uses and is unmistakably labeled as demo data (`source: demo`). A scoped reset action deletes only simulator-created sales — never manual, CSV, or webhook data — behind an explicit confirmation dialog, and is unavailable when `APP_ENVIRONMENT=production`.
...
- **Imports & Connections** — CSV sales-export import (preview → confirm), and the payment-webhook connection status/description (the hands-on demo now lives in the Local Demo Area above; the technical signed-request walkthrough for developers stays below, in "Webhook signing").
