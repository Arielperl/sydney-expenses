...
- **Support requests and staff console** — owners and managers use the button in Data Import to send a request tied to their current business. Staff sign in at `https://support.sydneyexpenses.com/support/login`; the separate support account has the database-backed `support` role and can view requests, update their status, and add/remove Grow or Cardcom for a business. The `admin` role has those permissions plus user-access deletion; support cannot list or delete users. A sole business owner cannot be deleted before ownership is transferred. The old `/admin` screen and API are gone. Neither the separate hostname nor an unlisted route is treated as an authorization control: the server checks the role on every staff request. Deleted auth identities retain a disabled, anonymized app-account tombstone to reject any still-valid access token while business sales remain intact.
