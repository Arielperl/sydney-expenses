...
**A sales/revenue product with automatic payment ingestion as the primary flow, document-issuance tracking, an exception center, and an AI assistant that reasons correctly about gross vs. net revenue vs. profit.** Sales arrive automatically — via a signed demo payment webhook or a CSV sales-export import — and the app attempts to issue each successful sale's customer document automatically (a demo-only adapter today). A receipt/invoice can also be attached to a sale after the fact by importing a photo of a previously issued document. The complete flow runs end-to-end, in Hebrew (default) or English, with decimal-safe money handling. See "Automatic sale ingestion" below for what's real, what's simulated, and what a real payment/invoicing provider would still need to add.
...
│   │   ├── services/documents/   DocumentProvider adapter boundary for issuing a customer receipt/invoice (demo-only today)
...
The target flow is: **a customer pays → a sale is created automatically → the app attempts to issue the customer's document automatically → the sale and its document status appear immediately on the dashboard and sales list.**
...
2. If the payment succeeded, Sydney attempts to issue the customer's receipt/tax document right away via a pluggable `DocumentProvider` adapter. Today only a demo adapter exists (see "Document issuance" below) — it's honestly labeled as a demo everywhere it appears, never presented as a real tax document.
...
**What's a demo, explicitly:** the webhook provider (`demo-pay`) is a stand-in for what a real payment provider or POS integration would send — there is no live payment connection, no OAuth, and no real financial institution involved. Likewise, **no real invoicing/tax-receipt provider is connected** — `MockDocumentProvider` is the only implementation, and it's labeled as a demo everywhere it's shown in the UI. The Imports & Connections page says so explicitly, not just here.
...
- **Sales** — search, filter by status/date, edit, delete, view an attached document, or import a historical document for a sale that's still missing one.
...
- **Imports & Connections** — CSV sales-export import (preview → confirm), and the payment-webhook demo connection status/instructions.
...
- `pytest` (backend, 266 tests) — sale validation (currency normalization, VAT-vs-gross-amount, non-finite rejection), sale CRUD API, decimal-precision dashboard math, mock/OpenAI/local extraction (all against fakes — no real network calls in any automated test), image-format verification, the payment webhook (signature/timestamp/body-size checks, idempotent delivery including a simulated concurrent-race test, refund reducing revenue correctly, partial refund reducing net amount correctly, only-succeeded-counts-as-revenue), CSV import (parsing, duplicate detection, document auto-attempt on imported sales), document import for an existing sale (extraction-failure-still-attaches, 404 for an unknown sale, never creates a new sale), the exception center (each of its four sections independently), and the AI assistant's tools (revenue/gross/VAT/fees/top-services/trend, refund-aware totals). All passing.
- `npm test` (frontend, 62 tests), `tsc -b`, `oxlint`, `npm run build` — all clean. Covers the sale form's validation schema, the sales list (search/edit/delete/empty-state), manual sale creation, the exception center's four sections and their action links, and the historical-document-import page (target-sale fetch, successful import, extraction-failure-still-attaches).
- `alembic upgrade head` verified on a fresh temporary SQLite database (all migrations, including the one that drops the old `expenses`/`receipt_uploads` tables and creates `sales`) and separately on a database carrying data forward from the prior migration.
- Manual end-to-end verification in the browser: signed payment webhook → sale appears on the dashboard and sales list within seconds → AI assistant correctly reports the new revenue → document status shows as issued (demo) → refund via the API reduces the dashboard's revenue total to zero.
...
- Code-split the frontend bundle (currently a single ~285 KB gzipped chunk, flagged by the Vite build but not a functional issue at this scale).
