...
Sydney (**מנהל הכנסות מבית Sydney**) is an AI-ready sales and revenue management product for Israeli small businesses. Sales can be recorded manually, imported from a CSV export, or ingested automatically from a connected Grow account, and appear immediately on the dashboard and sales list, remaining isolated to the authenticated business. The app tracks whether a receipt/tax document has been attached, and an AI assistant answers questions about revenue, VAT, fees, and refunds using only that business's data. Direct Cardcom ingestion is the next active integration milestone.
...
**No synthetic documents:** a Grow-ingested sale goes through the exact same document-issuance path as any other sale source. Production keeps `DOCUMENT_PROVIDER=disabled` until a real invoicing provider exists — a Grow sale's document stays truthfully `pending`, never a synthetic `DEMO-*` number, and this app never claims a receipt or tax invoice was generated.

**Verification status:** this integration has been structurally verified against Grow's own published payload examples (used verbatim as test fixtures — see `backend/tests/fixtures/grow_payloads.py`) and a full automated test suite, but is **not** considered live-verified until a real Grow account sends it a real transaction. See the deployment notes for the exact remaining step.
...
**Declined payments:** confirmed directly by Cardcom's representative, Cardcom *can* send a webhook notification for a declined transaction. When the server-to-server verification confirms a decline, it is recorded as a failed sale — visible in the connection's activity and the Exception Center — and is **never** counted as revenue. Whether Cardcom actually sends a decline notification for a given terminal is controlled by that terminal's own settings in the Cardcom dashboard (an external configuration step, not something this app can turn on).
...
**No synthetic documents:** a Cardcom-ingested sale goes through the exact same document-issuance path as any other sale source. Production keeps `DOCUMENT_PROVIDER=disabled` until a real invoicing provider exists — a Cardcom sale's document stays truthfully `pending`, never a synthetic `DEMO-*` number, and this app never claims a receipt or tax invoice was generated from a Cardcom payment.

**Verification status:** this integration has been structurally verified against Cardcom's own published `GetLpResult` response examples (used verbatim as test fixtures — see `backend/tests/fixtures/cardcom_payloads.py`) and a full automated test suite covering every documented payload variant, both content types Cardcom can deliver, declines, pending/unknown statuses, malformed payloads, wrong content type, IP allowlist enforcement, verification failure and safe reprocessing, and duplicate delivery — but is **not** considered live-verified until a real webhook is received from the Cardcom test account and confirmed end-to-end. No real Cardcom transaction has been sent as part of this work. See the deployment notes for the exact remaining step.
...
- `pytest` (backend, 524 tests) — including business isolation, development-only integration gates, signed (demo-pay), source-IP-verified (Grow), and server-to-server-verified (Cardcom) webhook routing, the Grow and Cardcom payload adapters against each provider's own published examples, the durable webhook-event inbox (received/processed/duplicate/failed/rejected) and safe provider-aware reprocessing, encrypted Cardcom credential storage, persisted assistant conversations, sale validation, VAT calculation, ingestion idempotency, CSV/document import, dashboard math, exception handling, security controls, and the complete Alembic migration chain. All passing.
- `npm test` (frontend, 123 tests), `tsc -b`, `oxlint`, `npm run build` — all passing. Coverage includes authentication, the Grow connection panel and the separate Cardcom connection panel (owner/manager/viewer visibility, copy/rotate/delete, activity including rejected events, safe reprocessing, absence of demo/test wording), persisted assistant conversations, CSV import, sales, Sale Details, dashboard, exception center, and historical-document import. `oxlint` currently reports pre-existing, non-blocking React warnings (none introduced by the Cardcom work) and no errors.
...
- **Cardcom integration is structurally verified** (Cardcom's own published `GetLpResult` examples as test fixtures, full automated suite covering every documented payload variant, production-config fail-fast checks) but **not live-verified** — no real transaction has been sent from the Cardcom test account, and none was sent as part of this work. See "Cardcom connection" above for what remains.
